...
public class zeroPath{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //solve your question in here for us
            //max and min value nikalni hai bas bois!! do this only!!!
            int[][] max = new int[n][m];
            int[][] min = new int[n][m];
            ...
            //now simple for loops will do the thing for us! No worries for us bois!
            for(int i = 1; i < n; i++){
                max[i][0] = arr[i][0] + max[i - 1][0];
                min[i][0] = arr[i][0] + min[i - 1][0];
            }

            for(int j = 1; j < m; j++){
                max[0][j] = arr[0][j] + max[0][j - 1];
                min[0][j] = arr[0][j] + min[0][j - 1];
            }

            //after this is done
            for(int i = 1; i < n; i++){
                for(int j = 1; j < m; j++){
                    max[i][j] = Math.max(arr[i][j] + max[i - 1][j], arr[i][j] + max[i][j - 1]);
                    min[i][j] = Math.min(arr[i][j] + min[i - 1][j], arr[i][j] + min[i][j - 1]);
                }
            }



            int maxVal = max[n-1][m-1];
            int minVal = min[n -1][m - 1];
