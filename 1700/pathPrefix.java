...
public class pathPrefix{
    ...
    public static void main(String[] args) {
        ...
        while(test--> 0){
            int n = sc.nextInt();
            ArrayList<Triplet>[] graph = new ArrayList[n + 1];

            for(int i = 0; i <= n ; i++){
                graph[i] = new ArrayList<>();
            }

            //after this take on the input, and make the answer
            //each edge shouold store the connection and two weights as well
            for(int i = 0; i < n-1; i++){
                int parent = sc.nextInt();
                int a = sc.nextInt();
                int b = sc.nextInt();

                graph[parent].add(new Triplet(i+2, a, b));
                graph[i+2].add(new Triplet(parent, a, b));
            }

            int[] answer = new int[n + 1];
            int[] vis = new int[n + 1];

            //this should be pretty simple for sure
            vis[1] = 1;
            solver(vis, answer, 1, graph, new ArrayList<>(), 0, 0);
    ...
    public static void solver(int[] vis, int[] answer, int curr, ArrayList<Triplet>[] graph, ArrayList<Long> list, long last, long csum){

        //update the answer for the given piece for real now
        if(curr != 1){
            //update the answer using binary search bois, hehehehehehe
            answer[curr] = finder(list, csum);
        }


        for(Triplet t : graph[curr]){
            int connect = t.a;
            if(vis[connect] == 1) continue;

            list.add(last + t.c);
            vis[connect] = 1;
            solver(vis, answer, t.a, graph,list, last + t.c, csum + t.b);

            list.remove(list.size() - 1);
        }
    }

    public static int finder(ArrayList<Long> list, long csum){
        //find how bigger is csum from the given sums, simpel
        //index bro
        int ans = -1;
        int left = 0;
    ...
    //now we should make the equation to solve this question
