...
public class gambling{
    ...
    public static void main(String[] args) {
        ...
        //code here
        //we have to print the number as well as the range with it
        //pretty annoying questions for sure, but let's do these no?
        ...
        while(test --> 0){
            ...
            //let's try to fix this, hashmap and indexes, simple as that
            ...
            for(int num : map.keySet()){
                ArrayList<Integer> clist = map.get(num);
                //check for this num if answer is possible or not
                int cl = clist.get(0);
                int cnt = 1;

                for(int i = 1; i < clist.size(); i++) {
                    int cidx = clist.get(i);
//                    int diff = cidx - cl;c
                    if ( (2 * cnt - (cidx - cl + 1)) < 0) {
                        cl = cidx;
                        cnt = 1;
                    } else {
                        //otherwise
                        cnt++;
                        int cans = 2 * cnt - (cidx - cl + 1);
                        if (cans > ans) {
                            best = num;
                            l = cl;
                            r = cidx;
