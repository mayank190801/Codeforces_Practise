...
public class rorobot{
    ...
    public static void main(String[] args) {
        ...
        //how to make segment trees, use segment trees for this question
        //then check if they can ever meet or not
        //it's a simple question for sure, just a lot of code for sure
        //which is pretty annoying for sure
        segmentTreeLazy seg = new segmentTreeLazy(arr);
        seg.buildTree(0, m-1, 1);

        //after this one should do something dope
        int q = sc.nextInt();
        for(int i= 0; i < q; i++){
            int x1 = sc.nextInt();
            int y1 = sc.nextInt();
            ...
            //if they can meet cols wise
            //if they can meet hight wise 
            //if they can connect to each other
            //three checks and you are good to go!
            boolean flag = true;
            int colDiff = Math.abs(y1 - y2);
            if(colDiff%k != 0) flag = false;

            //after this check heightwise
            int rowDiff = Math.abs(x1 - x2);
            if(rowDiff%k != 0) flag = false;

            //now most important check to be used! (think about it)
            //umm if they are above the max! and below the bottom
            if(colDiff > 1){
                int max = (int) seg.queryLazy(0, m-1, min(y1, y2) + 1, max(y1, y2) - 1, 1);

                //now check if they can even meet or not, the most important check in here for sure
                //this marks the start of your coding journey brother, please solve it asap
                //and never give up!! heheheheheh, you should be pretty good at coding
